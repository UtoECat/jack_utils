...
static int buff_except(ju_buff_t* b, ssize_t v) {
	if (v > 0) { // overflow
		// move backward
		C(S,_move)(b, -v);
#ifdef BUFFER_DEBUG
...
JBU_API void C(S,_except)(ju_buff_t* b, int (*cb) (ju_buff_t*, ssize_t)) {
	b->except = cb;
}
...
JBU_API size_t C(S,_remove)(ju_buff_t* b, void* dst, size_t s) {
	s = MIN(s, b->len);
	if (s > b->pos) {
		if (b->except) b->except(b, (ssize_t)b->pos - s);
		// if exception not fixed this :/
		if (s > b->pos) s = b->pos;
	}
	if (s == 0) return 0;
	memcpy(dst, b->data, s);
	C(S,_move)(b, -((ssize_t)s));
	return s;
}
JBU_API void   C(S,_move  )(ju_buff_t* b, ssize_t v) {
	if (v > (ssize_t)b->len) v = b->len;
	if (-v > (ssize_t)b->pos) v = -b->pos;
...
JBU_API size_t C(S,_read )(ju_buff_t* b, int fd, size_t s) {
	...
	if (s == 0) return 0;
	ssize_t r = read(fd, b->data + b->pos, s);
	if (r > 0) b->pos += r;
	return r;	
}
